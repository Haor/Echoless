...
读数随控制器锁定逐渐收敛到零
- **通用降噪** —— AEC3、NVAFX 与纯 AEC 的 LocalVQE v1.4 可在 `WEBRTC`、
`RNNOISE`、`OFF` 间选择;LocalVQE v1.2/v1.3 自带降噪,会锁定外接降噪以免重复处理
...
抑制,并可选 AGC。CPU 占用低,48 kHz 原生;需要时可在其后选择 WebRTC 或 RNNoise
通用降噪节点。
...
v1.2/v1.3 已包含降噪,选择它们时外接降噪会锁定为 `OFF`;纯 AEC 的 v1.4 可继续选择
`WEBRTC`、`RNNOISE` 或 `OFF`。
