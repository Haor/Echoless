...
1. `NoiseMode`、LocalVQE 能力矩阵和 processor manifest 已落地，未知 LocalVQE 模型按
   `OFF` 保守处理。
2. WebRTC NS 已从 AEC3 拆成通用节点，产品路径中的 AEC3 内部 NS 固定关闭。
3. RNNoise 已固定到 Xiph 官方提交和完整模型，使用静态 C 构建边界，逐帧处理无堆分配。
4. 主页面三段式 UI、LVQE 能力标签、说明文案和原子状态转换已落地。
5. AEC3 私有 `set_aec3_ns` 热控与事件已删除；CLI `--ns` 保留为通用 WebRTC 节点入口。
6. 本地根 workspace、AEC3 workspace、前端和 Tauri 后端门禁已通过；跨平台打包由功能
   分支 CI 验证。
