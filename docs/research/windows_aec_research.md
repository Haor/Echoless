...
> - **MVP**:WASAPI loopback + USB 麦克风 WASAPI capture + WebRTC AEC3(或先用 SpeexDSP) + VB-Cable / VAC 输出给 Discord。
> - **成品**:同一套用户态音频引擎 + 自研虚拟麦克风驱动 / 设备,Discord / VRChat 选择 "AEC Microphone"。
...
> **WebRTC AEC3 + WASAPI 双采集 + drift-aware 对齐 + 48 kHz stereo reference + 10 ms pipeline + MVP 用 VB-Cable,成品用自研虚拟麦克风。**
>
...
端到端:`WASAPI loopback 48 kHz stereo → Timestamped render ring + drift aligner → AEC3 AnalyzeRender() + USB mic 48 kHz mono → AEC3 ProcessCapture() → Postprocess(limiter / optional NS / AGC) → VB-Cable MVP 或自研虚拟麦克风 → Discord / VRChat`。构建见 §4.1;工程上建议封装自己的小 C API(`my_aec_create` 等,见 §4.1)。
