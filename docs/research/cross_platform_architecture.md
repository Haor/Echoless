...
| 主语言 | **Rust**(核心 + 平台后端) | 与 sonora 主线一致 |
| 处理方案(当前两种) | **经典 AEC3(sonora)** 与 **LocalVQE**,统一为 `EchoProcessor` 节点 | 用户确认范围 |
...
| 处理器:经典 AEC3 | **sonora(纯 Rust,默认)** | 跨平台、纯 Rust、避开 GN/depot_tools(C++ AEC3 的「硬墙」)。实现 `EchoProcessor` |
...
**一句话技术栈:** Rust + 统一 `EchoProcessor`(sonora 经典 AEC3 / LocalVQE,可单开可串联可组合可扩展)+ rubato(drift/边界 SRC)+ windows/coreaudio(I/O)+ 现成虚拟声卡(MVP,UX 仿 MicYou)+ CLI 先行、Electron 后期(经 daemon)。**核心与前端解耦,处理方案插拔。**
...
│     现有节点: ┌ SonoraAec3   (经典 AEC3,48k/stereo-ref)      │
│              └ LocalVqe      (端到端,16k/mono;可单开)        │
│     未来加更多方案 = 再写一个 impl EchoProcessor,核心不变      │
└───────────────▲──────────────────────────────────────────────┘
                │  trait: AudioSource / AudioSink / MonotonicClock
        ┌───────┴────────┬───────────────────────┐
┌───────┴──────┐  ┌──────┴───────────────────┐
│ echoless-hal-win  │  │ echoless-hal-mac              │   ← 唯一有平台代码
│ WASAPI mic   │  │ CoreAudio AUHAL mic      │
│ WASAPI loop  │  │ Process Tap (系统音频)    │
│ WaveRT/VBC   │  │ AudioServerPlugin/BlackH │
│ QPC clock    │  │ mach_absolute_time clock │
└──────────────┘  └──────────────────────────┘
```

**组合示例(配置驱动,运行时可切):**
- 单开经典:`mic → SonoraAec3 → out`
- 单开 LocalVQE:`mic → LocalVqe → out`
- 串联(经典主消 + 端到端兜底):`mic → SonoraAec3 → LocalVqe → out`
- 未来:`mic → SonoraAec3 → XxxResidual → LocalVqe → out`(任意顺序/任意数量)
...
├── crates/
│   ├── echoless-hal/             平台无关 trait + 类型(AudioPacket/Format/Flags/Clock)
│   ├── echoless-hal-win/         #[cfg(windows)]            WASAPI + WaveRT + QPC
│   ├── echoless-hal-mac/         #[cfg(target_os="macos")]  CoreAudio + Process Tap + HAL插件 + mach
│   ├── echoless-processors/      EchoProcessor trait + ProcessorChain + IoSpec(边界 SRC/downmix)
│   │     ├ sonora_aec3.rs   经典 AEC3 节点(包 sonora)
...
│   └── echoless-recorder/        Phase 0 采集(已存在,重构成基于 echoless-hal,自动跨平台)
└── vendor/  sonora · LocalVQE   (git submodule / path 依赖)
```

依赖方向单向:`echoless-cli/echoless-daemon → {echoless-hal-win|echoless-hal-mac} → echoless-hal`,`echoless-cli → echoless-core → echoless-processors → echoless-hal`。核心永不依赖平台 crate;**前端(CLI/Electron)只透过 `echoless-core::ControlApi` 访问,不直接碰处理器/HAL**。新增处理方案 = 在 `echoless-processors` 加一个 `impl EchoProcessor`,其余 crate 不动。
...
/// 虚拟麦克风输出(MVP=写进 VB-Cable/BlackHole;产品=原生驱动桥)
...
// ===== echoless-processors:统一回声处理节点(sonora 与 LocalVQE 都实现它)=====
...
pub struct NodeConfig  { pub kind: String, pub params: toml::Value }  // kind: "sonora_aec3" | "localvqe" | 未来…
...
| 虚拟麦输出 | MVP:VB-Cable;产品:WaveRT 驱动(simpleaudiosample 派生,主文档 §7.3) | MVP:BlackHole;产品:AudioServerPlugin(用户态,无内核) |
...
| AEC3 | **sonora**(共用) | **sonora**(共用,2400+ C++ validation 跑在 macOS) |
...
| 组合 | 链 | 适用 |
|---|---|---|
| 单开经典 | `mic → SonoraAec3 → out` | 48k/stereo-ref/robust DTD,CPU 最低,默认起点 |
| 单开 LocalVQE | `mic → LocalVqe → out` | 端到端 AEC+NS+dereverb;非线性/混响强、但 16k mono、double-talk ERLE 仅 8.5dB |
| 串联(经典→端到端) | `mic → SonoraAec3 → LocalVqe → out` | 经典先把线性回声/双讲压住,LocalVQE 收非线性/混响残余 |
| 未来扩展 | `mic → SonoraAec3 → Xxx → LocalVqe → out` | 加新 `impl EchoProcessor` 即可,任意顺序/数量 |

**chain 自动处理的事(节点不关心):**
- **边界 SRC + 声道适配**:SonoraAec3 在 48k(near 1ch / far 2ch),LocalVqe 在 16k mono;chain 在两节点之间用 rubato 做 `48k↔16k` + downmix。
...
### 7.1 为什么**不做** AEC3 内置 neural REE(也不为此改造 sonora)

用户已明确:我们要的是 **LocalVQE 作为独立可组合节点**,不是 AEC3 内置的 `NeuralResidualEchoEstimator` 深度集成。两者是不同的东西,这里记录为何放弃后者(2026-06 源码评估,以免日后重复纠结):

- **改造 sonora 技术上可行但不值得**:`sonora-aec3/src/residual_echo_estimator.rs:179` 明确跳过 neural REE;neural REE 需要的中间状态(`s2_linear/y2/e2/r2/dominant_nearend/时域 y,e/render block`)sonora 其实都已算好(`echo_remover.rs:225-354`,命名对齐官方 M145),纯代码 port 约 2-3 周。
- **真瓶颈与 sonora 无关**:① **AEC3 ML-REE 训练权重 Google 不公开**(只在 chromium 内部),自训需数月;② LSTM+Hanning+PFFFT+0.15 次幂,数值对齐无 golden test。无权重则一切是空架子。
- **LocalVQE 的 GGML 不能复用为 neural REE**——它是 DeepVQE 衍生的端到端模型(替代整个 AEC3+NS),不是 AEC3 的 residual mask,两条不相交的路。

**结论:neural REE 整个移出当前与可见未来的目标。** LocalVQE 通过 `EchoProcessor` 串联即可获得「经典 + 神经」的组合收益,无需深度集成。**唯一保留的低成本后门(~1 天,可选):** 给 sonora-aec3 留一个 `NeuralResidualEchoEstimator` trait + 扩 `ResidualEchoInput` 的 hook 不实现,纯粹为「万一哪天拿到权重」留口子——非当前任务。
...
### 8.2 虚拟麦克风输出 = AudioServerPlugin(比 Windows 简单)

- **用户态 HAL 插件**(跑在 `coreaudiod`,放 `/Library/Audio/Plug-Ins/HAL`),**无内核驱动、无 EV 内核签名**,门槛远低于 Windows WaveRT。自用场景只需 notarization 甚至关 SIP 即可加载。
- **MVP:直接用 BlackHole 作输出端**——我们的 app 把 clean 人声播到「BlackHole 输出设备」,Discord/VRChat 选「BlackHole 输入」即可。与 Windows MVP 用 VB-Cable 完全对称,`AudioSink` 同一接口。
- **产品自研虚拟麦的 user→plugin 喂音频通道**(XPC/shared memory)是和 Windows 一样的自研缺口(主文档 §7.3 同理),Phase 4 再做。
...
|---|---|---|---|
| **P0 采集** | ✅ echoless-recorder(已交付) | 加 macOS HAL(AUHAL mic + Process Tap ref) | OwnedPacket / sidecar 格式 |
| **P1 离线评测** | 同一套 harness | 同一套 | AECMOS_local + sonora/LocalVQE 对比(经 EchoProcessor 离线跑) |
| **P2 实时 MVP** | WASAPI loop + VB-Cable | Process Tap + BlackHole 输出 | echoless-core + ProcessorChain;**echoless-cli**(TOML 选链) |
| **P3 处理器成品** | sonora 节点 + LocalVQE 节点,单开/串联可配 | 同 | EchoProcessor 全跨平台共用;运行时换链 |
| **P4 原生 I/O** | WaveRT 虚拟麦 | AudioServerPlugin 虚拟麦 | — |
| **P5 前端 + 打磨** | stereo/tail/drift 强化 | 同 | **Electron + echoless-daemon(JSON-RPC)**;主文档 §11 Phase 5 |

**节奏要点:** ① P2 两平台都用现成虚拟声卡兜 I/O,核心 pipeline 一次写成两平台跑通,原生 I/O 压到 P4;② **前端 CLI 先行(P2 起),Electron 留到 P5**——但 P2 就把 `ControlApi` 边界划好,CLI 直接内嵌、Electron 后期经 daemon,二者共用同一控制面(§14)。
...
- **仓库级证据(sonora/LocalVQE/AEC3 file:line)** → 以 `reference_repos_exploration_report.md` 为准。
...
- 配置即「设备选择 + 处理链」:
  ```toml
  mic    = "default"          # 或设备名/ID
  ref    = "system"           # Win=loopback, mac=Process Tap
...
*2026-06-05 创建,2026-06-05 更新(处理器统一为可组合 EchoProcessor 图;neural REE 移出目标;前端 CLI 先行 Electron 后期;加入 MicYou 参考);基线 Windows 10/11 + macOS 14.4+,处理方案仅经典 AEC3(sonora)+ LocalVQE,本地自用无 license 约束。*
