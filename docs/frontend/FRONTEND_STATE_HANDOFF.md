...
2. LocalVQE assets 探针已扩展为 model + native readiness;构建环境提供模型和动态库时,
   `pnpm prepare:tauri-assets` 会复制到 `resources/localvqe/`。
...
- `localvqe_assets()` 已返回 bundled/downloaded models、`native_ready`、`library_path`、`native_dir`、
