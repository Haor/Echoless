...
手动调整或运行中写入该值时可热控;完整 probe 流程仍会暂停主 runtime,因为 probe 需要独占
设备播放蜂鸣和录制。这个值会进入用户感知延迟估算:
...
- 当 `recommended_near_delay_ms > 0` 且结果稳定时,可自动填入顶层 `near_delay_ms`。手动调节
  `near_delay_ms` 可运行中热控;完整 probe 流程仍会暂停主 runtime,因为 probe 需要独占设备。
