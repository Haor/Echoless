...
| **`nvafx_download_install`** | `runtime_dir?` | ⛔ **stub** | **见下,请实现** |

`nvafx_download_install` 当前返回错误:
"RTX 下载安装器尚未接入(等公共 release 上线);当前请用 Local zip。"
仓库已公开,请实现真实下载安装。
...
前端会把 `nvafx_download_install` 的 stub 换成 shell 这个子命令,直接回传末尾的 doctor JSON。
