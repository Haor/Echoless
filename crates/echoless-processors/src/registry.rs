...
use crate::{localvqe::LocalVqe, passthrough::Passthrough, sonora_aec3::SonoraAec3, EchoProcessor};
...
pub fn build(kind: &str) -> anyhow::Result<Box<dyn EchoProcessor>> {
    Ok(match kind {
        ...
        other => anyhow::bail!("未知处理器 kind: {other}(可用: passthrough / sonora_aec3 / localvqe)"),
    })
}

/// 已注册的处理器种类(供 CLI/前端列出)。
pub fn kinds() -> &'static [&'static str] {
    &["passthrough", "sonora_aec3", "localvqe"]
}
