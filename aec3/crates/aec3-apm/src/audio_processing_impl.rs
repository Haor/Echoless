...
use aec3_core::config::EchoCanceller3Config;
use aec3_ns::config::{NsConfig, SuppressionLevel};
use aec3_ns::noise_suppressor::NoiseSuppressor;
...
pub(crate) struct AudioProcessingImpl {
    ...
    // Echoless: 外部注入的 AEC3 config(None=用默认)。见 initialize_echo_controller。
    aec3_config_override: Option<EchoCanceller3Config>,
}
...
impl AudioProcessingImpl {
    ...
    pub(crate) fn with_config(config: Config) -> Self {
        let mut apm = Self {
            ...
            aec3_config_override: None,
        };
        apm.initialize();
        apm
    }

    /// Echoless: 注入自定义 AEC3 config。必须在 `initialize()` / `initialize_with_config()`
    /// 之前调用才会生效(echo_controller 在 initialize 时按此 config 构建)。
    pub(crate) fn set_aec3_config_override(&mut self, config: Option<EchoCanceller3Config>) {
        self.aec3_config_override = config;
    }
    ...
    fn initialize_echo_controller(&mut self) {
        ...
        // Echoless: 若注入了自定义 config 则以它为基(mono + multichannel 同源),否则维持上游默认。
        // 无论哪种,都保留高层 EchoCanceller.transparent_mode 旋钮的语义。
        let (config, multichannel_config) = match &self.aec3_config_override {
            Some(cfg) => {
                let mut c = cfg.clone();
